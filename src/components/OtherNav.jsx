...
const OtherNav = ({ companyName, className }) => {
      ...
      <div className="fixed w-screen top-0 z-50 bg-white backdrop-blur-sm h-20 max-md:h-25 max-md:px-3 px-30 flex justify-between items-center shadow-sm">
        <div className="flex items-center text-black gap-3">
          <img
            src={logo}
            alt="company logo"
            className="w-[50px] max-md:w-[60px]"
          />
        ...
        {/* Desktop Navigation */}
        <ul className="max-md:hidden text-blue-900 font-semibold flex gap-6 text-md z-90 relative">
          {navItems.map((item) =>
            item.dropdown ? (
              <li
                key={item.name}
